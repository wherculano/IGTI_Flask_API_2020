...
# configurando Flask_Alchemy
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///data.db'

# configurando modificações automaticas da sessão
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# configurando se quero propagar as excessões de banco para o SQLAlchemy
app.config['PROPAGATE_EXCEPTIONS'] = True

# secret key da app
app.secret_key = 'UmaSenhaQualquer'


# deixando o banco criado antes do primeiro request
@app.before_first_request
def create_tables():
    alchemy.create_all()


@app.route('/', methods=['GET'])  # rota e metodos aceitos, neste caso só GET
def home():
    return 'API Funcionando', 200  # retorno o código de status HTTP


@app.route('/show', methods=['POST'])
def create_show():
    request_data = request.get_json()  # retorna o json/dict
    new_show = show.ShowModel(request_data['name'])
    print('NEW:', new_show)
    new_show.save_to_db()
    result = show.ShowModel.find_by_id(new_show.id)
    print('RESULT:', jsonify(result.json()))
...
if __name__ == '__main__':
    ...
    app.run(port=5000)  # porta padrão
